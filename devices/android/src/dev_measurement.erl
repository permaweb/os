%%% @doc Common HandEE hardware-measurement protocol.
%%%
%%% This device owns the standard measured subject:
%%% `#{<<"system">> => ~system@1.0/all, <<"node">> => ~meta@1.0/info}'.
%%% `~handee@1.0' supplies Android Keystore/StrongBox evidence and recipient
%%% handling. Measurements expose facts and provenance as signed AO-Core
%%% messages.
...
-define(DEFAULT_DEVICES, [<<"handee@1.0">>]).
...
publish_cached_measurement(Msg, Opts) ->
    {ok, _ID} = hb_cache:write(Msg, Opts),
    Msg.
...
fetch_linked_credential(NodeURL, CredentialID, Opts) ->
    PeerOpts = lib_handee_peer_http:peer_opts(NodeURL, Opts),
    materialize_peer_value(
        response_body(
            lib_handee_peer_http:get(
                NodeURL,
...
verify_peer_url(Url, Req, Opts) ->
    with_raw_ok(fun() ->
        PeerOpts = lib_handee_peer_http:peer_opts(Url, Opts),
        Boot = peer_measurement_payload(response_body(
            lib_handee_peer_http:get(Url, <<"/~measurement@1.0/boot">>, Opts),
            PeerOpts), PeerOpts),
...
allow_rejected_peer_attestation(Req, Opts) ->
    truthy(first_defined([
        ...
        hb_opts:get(<<"allow-rejected-peer-attestation">>, undefined, Opts)
    ])).
...
peer_attestation_scope(Url, Boot, Fresh, Subject, Req, Opts) ->
    #{
        ...
        <<"secret-recipient-id">> => secret_recipient_id(Subject, Opts),
        <<"consumer-scope">> =>
...
materialize_measurement(Msg, _Opts) when is_map(Msg) ->
    normalize_top_keys(Msg);
...
canonical_payload(Link, _Opts) when ?IS_LINK(Link) ->
    Link;
canonical_payload(Msg, Opts) when is_map(Msg) ->
    ...
    maps:from_list(
        [
            {Key, canonical_payload(Value, Opts)}
         || {Key, Value} <- hb_maps:to_list(Body, Opts),
            not detached_transport_key(Key)
        ]);
