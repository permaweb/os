...
6. **PCR 15 extension digest == `node_message_id_hex`**.
7. **Embedded `node_message` + id shape** (id decodes to 32 bytes).
...
- Trust anchor selection is operator-supplied via `--roots-dir`. A
  useful pointer: `agent/lapee` ships a corpus under
  `priv/tpm-interpret/root-cas/`. **Audit that corpus before use** — at
  time of writing, at least one file in it (`NUVOTON_NPCTxxx_ECC384_LeafCA_012110.pem`)
  is an intermediate, not a root, and this verifier will honestly
  refuse to treat it as a trust anchor.
...
Exit code: 0 if all seven checks pass, 1 otherwise. Each check prints
