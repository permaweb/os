...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: validate-andock-manifest.py MANIFEST")
    ...
    for key, input_path in INPUTS.items():
        if manifest.get(key) != digest(input_path):
            raise SystemExit(f"stale Andock template input: {input_path}")
