...
\author{%
  Claude Opus 4.7\\\small{Anthropic}
  \and
...
\begin{tikzpicture}[
  ...
  \node[layer, above=of fs, fill=green!12] (dev) {Devices, \texttt{trusted-device-signers}-verified at first load};
...
through the operator's \texttt{trusted-device-signers} set, through the
initiation of the computation and every input, and finally to the result.
The ``LapEE'' label in a higher-layer predicate (for example a
\texttt{zone@1.0} membership template) is not a vague tier marker but
the conjunction of every concretely-verifiable property in the
measurement. The chain binds the transcript to the boot conditional on
A1 --- the same conditional under which TDX or SEV-SNP guest
attestations bind their guest transcripts.
...
the appliance. A verifier policy is a small open data structure: expected
PCR values per layer, required Secure Boot or SNP policy bits, accepted
EK-chain anchors, required node-message fields, and any device-specific
hardware predicates. The reference implementation exposes named facts
and provenance rather than a global score. The same measurement can
therefore be accepted by one zone template and rejected by another
without re-attestation. Importantly, verifier policy is public and
consumer-chosen, not appliance-chosen --- the appliance just produces
signed evidence.
...
Nuvoton dTPM, INSYDE H2O firmware) boots from a USB stick to
measurement-serving. The envelope returned at
\texttt{/\~{}measurement@1.0/boot} contains the signed HyperBEAM node
message, the \texttt{\~{}system@1.0/all} machine report, and
device-native evidence. In the TPM path this evidence includes the EK
certificate read from TPM NV at handle \texttt{0x01C00002}, the AK
public key, a \texttt{TPM2\_Quote} over the boot PCR set, the firmware
TCG event log, the HyperBEAM runtime event log, PCR-15 replay tying the
boot subject to the quoted PCR state, and platform probes from DMI,
ACPI, \texttt{/proc/cpuinfo}, and kernel security state. The HyperBEAM
node wallet signs AO-Core results; the TPM AK signs the boot quote that
binds that node wallet and configuration to the measured boot.

\paragraph{Verifier output.} An external verifier can run offline
against the TPM portion of the envelope and report the load-bearing
...
\end{quote}
The in-tree \texttt{\~{}measurement@1.0} and
\texttt{\~{}measurement@1.0} and \texttt{\~{}tpm@2.0a} expose the same checks as
structured AO-Core messages plus a wider set of neutral facts. Zone
templates and external callers decide which facts are required; the
appliance does not collapse them into a universal score.
...
\texttt{\~{}measurement@1.0}, \texttt{\~{}tpm@2.0a}, and
\texttt{\~{}snp@1.0} devices ship in the LapEE repository root.
